...
const VideoDetailsClient = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const recRes = await TMDB_Api_Service.get(
          `/${type}/${id}/recommendations?&page=1`
        );
  ...
  return (
    <div className="relative">
      <div className="w-full absolute top-0 left-0 right-0 bottom-0 flex justify-center">
        <div className="w-[64px] bg-white h-2 rounded-full"></div>
      </div>
      <div className="bg-black w-full h-full absolute top-0 left-0 right-0 bottom-0 -z-10">
        <Image
          unoptimized
          ...
          height={1000}
          alt="bg"
          className="w-full h-full opacity-50 md:hidden object-cover"
        />
        <Image
          unoptimized
          ...
          height={1000}
          alt="bg"
          className="w-full h-full opacity-50 max-md:hidden object-cover"
        />
      </div>
      <div className="p-3 md:py-12 md:space-y-6 flex flex-col text-white max-h-[90vh] overflow-y-auto mt-6">
        <div className="flex flex-col md:flex-row bg-black backdrop-blur-lg border-gray-700 border p-6 rounded-xl bg-opacity-40 max-md:p-3 max-md:space-y-3 md:space-x-6 my-0 md:mx-12">
          ...
          <div className="md:w-2/3 text-white flex flex-col max-md:justify-between space-y-2">
            <div className="px-4 text-xs font-bold rounded-lg flex max-md:flex-col max-md:space-y-3 md:justify-between w-full">
              <div className="text-lg max-md:py-2 md:text-3xl font-medium rounded-lg">
                {(title || name).toUpperCase()}
              </div>
            </div>
            <div className="px-4 text-gray-300 text-sm hidden md:block font-light">
              {tagline}
            </div>
            ...
            <div className="flex space-x-8 items-center md:py-2 px-4">
              <div className="flex space-x-1.5 items-center">
                ...
                <p className="text-white md:text-md text-xs font-light">
            ...
            <div className="flex space-x-2 items-center md:py-1 px-4">
              {genres?.map((genre: any, i: number) => (
                <div
                  key={i}
                  className="flex flex-wrap space-x-2 items-center bg-white bg-opacity-30 rounded-md px-4 py-1"
                >
                  <p className="text-white md:text-md text-xs font-light whitespace-nowrap">
                    {genre?.name}
                  </p>
                </div>
              ))}
            ...
            <div className="md:py-2 px-4 flex md:max-w-[50rem] w-full h-full space-x-4 overflow-x-auto noScroll max-md:pb-2">
              {credits.cast
                .filter((credit: { profile_path: any }) => credit.profile_path)
                .map((credit: any, i: number) => (
                  <HoverCard key={i}>
                    ...
                    <HoverCardContent
                      sideOffset={-40}
                      side="bottom"
                      className="flex flex-col justify-center items-center p-2 rounded-lg bg-black/40 backdrop-blur-sm w-auto space-y-1.5"
                    >
                      <Image
                        unoptimized
                        src={
        ...
        <div className="md:px-4 max-md:pb-32">
          <MovieList
            isFull
...
export default VideoDetailsClient;
